...
export default class CustomDateFilters extends Component{
  ...
  customDateSearch() {
    ...
    urlParameters['f.'+defaults.customDateSettings.customDateField+'.filter'] = "["+moment(this.state.startDate).format('YYYY-MM-DDTHH:mm:ss')+"TO"+moment(this.state.endDate).format('YYYY-MM-DDTHH:mm:ss')+"]";
    ...
    for(let i=0, len = facetFields.length; i<len; i++){
      ...
      if(facetFields[i].dateRange){
        ...
        if((facetFields[i]['field'] == customDateField) && urlParameters[`f.${customDateField}.filter`]){
            urlParameters[`f.${facetFields[i].field}.range`].push("["+moment(this.state.startDate).format('YYYY-MM-DDTHH:mm:ss')+"TO"+moment(this.state.endDate).format('YYYY-MM-DDTHH:mm:ss')+"]");
        }
      }
      else{
        urlParameters['f.'+defaults.customDateSettings.customDateField+'.range'] = "["+moment(this.state.startDate).format('YYYY-MM-DDTHH:mm:ss')+"TO"+moment(this.state.endDate).format('YYYY-MM-DDTHH:mm:ss')+"]";
      }
    }
