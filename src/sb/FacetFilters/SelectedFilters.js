                  ...
                  let field = Object.keys(facet)[0];
                    const variable = facet[field].map((filter, index) => {
                      if(filter.rangeField){
                        return  filter.filterSelect ? <a href="" key={index} className="cdatefilter" onClick={(e) => this.toggleFilter(e,field, filter.fromValue)} title={filter.filterName}>{filter.filterName.replace(/&quot;/g, '"').replace(/&amp;/g, "&")}<span style={{marginLeft:"10px"}}><i className="fa fa-close"/></span></a> : null;
                      }
                      else {
