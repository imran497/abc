  ...
  render(){
    ...
    return (
      <div>
        {
          results.map((result)=>{
            return <Result key={result['no']} result={result} highlight={resultInfo.highlight} getDbResults={this.getDbResults} getEmailViewerFunc={this.getEmailViewerFunc} documentClick={this.documentClick} morelikethisClick={this.morelikethisClick}/>;
          })
        }
        {
...
function Result(props){
  ...
  if(result["ml_title"] && result["ml_title"].length > 0) {
    result.title = result["ml_title"];
  }
  ...
  }else if(result["ml_description"]){
    result.descriptionContent = result["ml_description"];
     ...
     <div className={contentType === "html"?"searchResult":""}>
       ...
       <div>
           ...
           }
           {
             (result.colname) &&
             <div className="d-inline-block mr-3">
               <span className="result-colname" style={{color:"#aba9a4"}}>Collection: </span>
     ...
     </div>
     {
       (contentType === "html") &&
         <div className="featuredImg">
         {
           (result['og:image'])
           ?
           <a href={result.contentUrl} target="_blank"><img src={result['og:image']} title={result['og:image']}/></a>
           :
           ""
         }
         </div>
