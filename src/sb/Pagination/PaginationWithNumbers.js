            ...
            }
            {
              this.state.pagesArray.map((val)=>{
                return <PaginationItem key={val} className={[(urlParameters.page === val || urlParameters.page === val.toString())?'active' :'',(!urlParameters.page && val === 1)?'active' :''].join(" ")}>
                  <PaginationLink href="#" title={val} onKeyDown={(e)=> this.keydown(e,val)} onClick={(e)=>{(urlParameters.page === val || urlParameters.page === val.toString())? this.currentPage(e): this.getPage(e,val);}} tabIndex={`${(urlParameters.page === val || urlParameters.page === val.toString())?"-1" :"0"}`}>{val}
                  </PaginationLink>
                </PaginationItem>;
              })
            }
