...
export const getInitialUrlParameters = (query) => {
  ...
  if(Object.keys(urlParameters).length === 0){
    ...
    urlParameters.sortdir = defaults.sortDirection;
    urlParameters.sort = defaults.sortButtons[1].field;
  ...
  if(sb_sort !== undefined && sb_sort !== null && sb_sort !== ""){
    ...
    urlParameters.sort = sortValues[0];
    urlParameters.sortdir = sortValues[1];
  }
                  ...
                  for(let fc = 0, lenfc = sbFacet["int"].length; fc < lenfc; fc++){
                    ...
                    if(sbFilter["from"] || sbFilter["to"]){  // IF FILTER IS DATE TYPE
                      if(sbFilter["to"]!==null){
                        ...
                        rangeFormat = "[" + moment(sbFilter["to"]).format('YYYY-MM-DDTHH:mm:ss') + "TO" +moment(sbFilter["from"]).format('YYYY-MM-DDTHH:mm:ss') +"]";
                      }
                      else{
