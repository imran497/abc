...
export default class AutoSuggestComponent extends React.Component{
  ...
  componentDidMount(){
    axios({
        method: 'post',
        url: pluginDomain+'/rest/v2/api/query/top',
        data: {
  ...
  componentWillReceiveProps(nextProps){
    if(nextProps.q.length >= 3){
      ...
      try{
        let regex = /[^\w]|_/g;
        console.log(this.props.q,nextProps.q,"Dfdsf")
        if(this.props.q !== nextProps.q && !regex.test(nextProps.q)){
          let autoSuggest = [];
          ...
          .then((response)=>{
            ...
            if(this.state.topquery.length>0){
              console.log(this.state.topquery,"dfsdf")
              ...
              return (
                  <li
                    ...
                    </span>
                    {
                      ...
                      :
                      <span dangerouslySetInnerHTML={{__html: this.highlightFunc(suggestSplit, this.props.q)}}>{}</span>
                    }
                  </li>
              );
            })
