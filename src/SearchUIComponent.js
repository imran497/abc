...
class SearchUIComponent extends Component {
  ...
  myHandler(e) {
    if (!($(e.target).parents('.navbarSort').length > 0)) {
      document.getElementById("myDropdown").classList.remove("show");
      document.getElementById("caret").classList.remove("open");
    }
  }
        ...
        if(alternateQuery.indexOf('"') >= 0 && parseInt(response.data['hits'], 10) === 0){
          ...
          parser.getResults(this.state.parameters);
            ...
            if(response.constructor !== Object || response.data.constructor !== Object){
              ...
              document.title = `Search Results: ${unescape(this.state.response.resultInfo.query).replace(/&quot;/g, '"').replace(/&amp;/g, "&").replace(/\\\//g, "/").replace(/\\\\\//g, "/")}`;
            }else{
              if(params.facetonly){
                // this.setState({
            ...
            document.title = `Search Results: ${unescape(this.state.parameters.query).replace(/&quot;/g, '"').replace(/&amp;/g, "&").replace(/\\\//g, "/").replace(/\\\\\//g, "/")}`;
          }
        }
      }).catch((error)=>{
        this.setState({
    ...
    return(
      <Fragment>
        <div className="searchGrid">
          <Row className="m-0 login-firstbar" id="loginDiv">
            ...
            <Col sm="12" className="px-4">
              <Row className={`height-50 ${response.results && response.resultInfo && response.resultInfo.hits >= 0 ?"spacingcss":""}`}>
                <div className="topbar_logo col-lg-2 col-sm-3" id="logoDiv">
                ...
                <div className={`topbar_search col-lg-8 col-sm-6 ${!parameters.query && !this.state.isLoading?"searchNoParams":""}`}>
                ...
                <div className="topbar_icons col-lg-2 col-sm-3">
                  {
                    (response.results && response.resultInfo && response.resultInfo.hits >= 0) &&
                      <div style={{margin:"11px 0 0"}}>
                        ...
                        <li className="navbarSort">
                          <button onClick={this.dropdownFunc} className="dropbtn">
                  ...
                  }
                  {
                    ...
                    <div className="header" style={{margin:"5px 0"}}>
                      <UncontrolledDropdown className="log-in header__top-item" id="logIn" nav>
                        <DropdownToggle onFocus={this.dropdownBlur}
                          aria-haspopup
                          caret
                          color="default"
                          id="navbarDropdownMenuLink"
                          nav>
                          {/*<img width="24" height="24" src={require('./images/user.png')}/>*/}
                          <i className="fas fa-user" />
                        </DropdownToggle>
                        <DropdownMenu aria-labelledby="navbarDropdownMenuLink" className="navbarDropdownMenuLink" right>
                          <DropdownItem>
                            <p className="dd-heading">Account</p>
                            <p className="dd-content">{localStorage.getItem("loginUserName")}</p>
                          </DropdownItem>
                          <DropdownItem id="logoutBtn" className="dropdown-content" onClick={e => this.logoutUser(e)}>
          ...
          <div id="results-container" className={(!defaults.facetsFiltersDisplay || response.facets === undefined || response.facets.length <= 0 || response.results === undefined || parseInt(response.resultInfo.hits, 10) === 0)?'resultNoFaacets':'resultWithFacet'}>
          {
          (response.results) && (
          ...
          <Fragment>
            <SelectedFilters facets={response.facets?response.facets:[]}/>
            {
              ...
              <SuggestAutoSearch resetActualQuery={this.resetActualQuery} resultquery={response.resultInfo.query.replace(/\\\//g, "/").replace(/\\\\\//g, "/")} actualquery={this.state.actualQuery.replace(/\\\//g, "/").replace(/\\\\\//g, "/")}/>
            }
            <div className="display-count-data">
              <div className={['resultsText padding-0',(parameters.XPC?"resultsForMore":"")].join(" ")}>
               ...
               {
               (response.resultInfo.query) && <span> for <b dangerouslySetInnerHTML={{__html: (response.resultInfo.query.replace(/\\\//g, "/").replace(/\\\\\//g, "/"))}}>{}</b>.</span>
               }
               {
          ...
          <Fragment>
            <div className="noResultsDiv" id="noResultsContainer" style={{padding: "0 1em 1em"}}>
                <p>Your search for - <b dangerouslySetInnerHTML={{__html: (response.resultInfo.query.replace(/\\\//g, "/").replace(/\\\\\//g, "/"))}}>{}</b> - did not match any documents.</p>
                {
                ((parameters.topQuery && parameters.topQuery === 'true' && parameters.page === "1") || (defaults.topQuery && parameters.page === "1")) &&
                <TopQueryComponent/>
                }
        ...
        <a className="back-to-top" href="" onClick={(e) => this.topScroll(e)} tabIndex="0">
